...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.22.9-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.22.9 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.22.9 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        if not _ham_status.get("ready"):
            ...
            _cache_key=("v110.3.22.9",int(len(_Sv)),int(_Sv.shape[1]),float(np.nanmean(_Jv[0])),_vh_cache_sha)
            if st.session_state.get("v32110_seq_key")!=_cache_key:
                with st.spinner("Construyendo atlas maestro Denver→OpenSim en frame 1 y retargeteando los 75 frames SKEL…"):
                    ...
                    # Hamner/OpenSim is always available as the kinematic rig.
                    _BV_full,_BF_full=build_hamner_sequence(
                        _hb.get("bodies",{}),_Jv,_JNv,_Sv,
                        _build_opt,max_faces_each=120
                    )
                    ...
                    if _vh_man and _vh_man.get("ready"):
                        try:
                            from visible_human_atlas import (
                                build_visible_human_muscle_sequence,
                                build_visible_human_bone_sequence,
                            )
                            # REAL segmented Denver lower-limb bones.
                            _DBV,_DBF=build_visible_human_bone_sequence(
                                _vh_man,_Jv,_JNv,_Sv,max_faces_each=1200
                            )
                            # REAL volumetric Denver muscles.
                            _VMV,_VMF=build_visible_human_muscle_sequence(
                                _vh_man,_Jv,_JNv,_Sv,max_faces_each=900
                            )

                            # Denver has no equivalent open full upper-body atlas here.
                            # Keep ONLY Hamner torso/arms/hands as visual fallback;
                            # lower-limb Hamner bones are hidden, not overlaid.
                            _upper_ids={
                            ...
                            _UBV,_UBF=build_hamner_sequence_filtered(
                                _hb.get("bodies",{}),_Jv,_JNv,_Sv,
                                _build_opt,_upper_ids,max_faces_each=120
                            )
                            _BV,_BF=merge_mesh_sequences(_DBV,_DBF,_UBV,_UBF)
                        except Exception as _ve:
                            st.warning(
                                "No se pudo preparar la anatomía Denver completa; "
                                f"se usa Hamner como fallback: {type(_ve).__name__}: {_ve}"
                            )
                            _BV,_BF=_BV_full,_BF_full
                    else:
                        _BV,_BF=_BV_full,_BF_full
                st.session_state["v32110_bone_vertices"]=_BV
                st.session_state["v32110_bone_faces"]=_BF
                st.session_state["v3228_denver_real_bones_active"]=bool(_DBV is not None and _DBF is not None)
            ...
            if st.session_state.get("v3228_denver_real_bones_active"):
                st.success(
                    "Representación anatómica activa: huesos Denver segmentados reales en miembro inferior + "
                    "músculos Denver volumétricos + rig OpenSim/SKEL. "
                    "Hamner queda visible solo en tronco/brazos/manos como fallback."
                )
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.22.9 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.22.9 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
